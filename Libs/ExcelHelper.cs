...
namespace Libs
{
    public partial class ExcelHelper
    {
        ...
        public static string getWorksheets(DataSet source)
        {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        foreach (DataColumn dc in dt.Columns)
                        {
                            if (dc.DataType == typeof(DateTime))
                                data = ((DateTime)dt.Rows[i][dc.ColumnName]).ToString("HH:mm:ss, dd-MM-yyyy");
                            else
                            {
                                //if (dc.ColumnName == "Nick")
                                //    data = DBCommon.strDecode(dt.Rows[i][dc.ColumnName].ToString());
                                //else
                                    data = dt.Rows[i][dc.ColumnName].ToString();
                            }
                            sw.Write(getCell(typeof(string), data));
                        }
                        sw.Write("</Row>");
                    }
                    sw.Write("\r\n</Table>\r\n</Worksheet>");
        ...
        public void ToExcel(DataTable dtInput, string filename, HttpResponse response)
        {
            var ds = new DataSet();
            ds.Tables.Add(dtInput.Copy());
